...
REM =====================================================================
REM eAISEDP 一键部署脚本 #19 三层贯通（基于 #18 最终版，新增 L3/L2/L1 三级管理）
REM 日期：2026-08-18
REM
REM 本脚本在 #18（整合 #13~#17 全部功能）基础上新增：
REM   ✅ 三层贯通: L3战略→L2项目群/项目→L1 Case 四级联动
REM   ✅ V4 迁移: 六新表+seed（Flyway 启动时自动执行）
REM      t_strategy / t_program / t_project / t_architecture_principle
REM      t_quality_gate_rule + Case 表加 project_id 外键
REM   ✅ 架构原则注入: 编排上下文自动注入（runtime 日志看 [Inject] 行）
REM   ✅ 质量门禁规则化: 删 GATE_ROLES 硬编码，升级为 t_quality_gate_rule 可配置（等价）
REM   ✅ 上行事件: Case done → Spring 事件 → 异步全量重算项目进度
REM   ✅ 灵活接入(P13): L3/L2 开关（分层设置页），场景A端到端/B从L2接入/C仅L1
REM   ✅ P0修复: @MapperScan annotationClass 限定防 Service 双注册
REM   ✅ 前端: 9 新页面 + menu.js 按 layers 过滤菜单（场景C 仅 L1）
REM
...
echo ═══════════════════════════════════════════════════════
echo   eAISEDP 三层贯通部署 #19（L3战略→L2项目群/项目→L1 Case）
echo   %date% %time%
echo ═══════════════════════════════════════════════════════
echo.
...
echo   √
echo.
echo   [Flyway] 启动时自动执行 V4（六新表+seed）:
echo     t_strategy / t_program / t_project / t_architecture_principle
echo     t_quality_gate_rule + Case 表 project_id 外键 + 种子数据
echo.

REM ============ Step 4: 数据库检查（Flyway 自动迁移 V4） ============
echo [4/7] 数据库检查...
...
echo   √ MySQL 正常（已清理失败迁移记录含 V4 草案兜底，Flyway 启动时自动迁移）
...
>> "%TMP_RT%" echo echo === 日志关注: Flyway V4迁移/加载角色数/[Inject]注入行 ===
