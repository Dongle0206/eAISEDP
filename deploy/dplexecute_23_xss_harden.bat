...
REM =====================================================================
REM eAISEDP 一键部署脚本 #23 XSS 加固收尾（安全复审 R2~R4 残余清零 + IMP-012 评估关闭）
REM 日期：2026-08-20 | Wave3 工程债
REM
REM 【本次交付】（纯前端 + 文档，后端零改动；全流程一键骨架不变）
REM   ✅ R2 协议中缀控制字符绕过修复: "java\tscript:" 经浏览器 URL 解析等价
REM      javascript: 但 startsWith 判不出——sanitize.js 归一（剥 \x00-\x20\x7f）
REM      后按协议白名单判定（http/https/mailto/ftp/相对路径/data:image 排除svg）
REM   ✅ R3 向量面加固: 标签黑名单增 base/svg/math/frame/frameset/applet
REM      （base 劫持相对 URL、svg xlink:href 与内嵌脚本、MathML 事件属性）
REM      + 属性黑名单增 xlink:href
REM   ✅ R4 CSS 面加固: 删 style 属性（钓鱼 UI/老浏览器 url(javascript:)）
REM      + formaction/srcdoc/srcset
REM   ✅ 先例统一: case-detail/artifact-view 内联 sanitizeHtml 收敛为公共
REM      sanitize.js（消除三处重复同弱点半径）；未加载时 try-catch 降级纯文本
REM   ✅ IMP-012 评估关闭: t_derivation 三条查询索引全命中（tenant/case 前缀），
REM      is_deleted 为索引后过滤且单组合行数小，高写表加索引写放大>收益，
REM      与全库逻辑删表口径一致——不建索引（结论留痕 kanban/changelog）
REM
...
echo ═══════════════════════════════════════════════════════
echo   eAISEDP 部署 #23 XSS 加固收尾（R2~R4 清零 + IMP-012 关闭）
echo   %date% %time%
echo ═══════════════════════════════════════════════════════
echo.
