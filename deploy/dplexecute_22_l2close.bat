...
REM =====================================================================
REM eAISEDP 一键部署脚本 #22 L2治理收口（标准库/模板库/数据治理/试用到期拦截）
REM 日期：2026-08-20 | case-20260820-L2治理收口
REM
REM 【本次交付】
REM   ✅ F1 标准库: 多版本状态机（draft→published→deprecated，发布自动取代）
REM      + 模板库（原地升版）+ 标准-门禁打通（打回展示"依据标准"）
REM   ✅ F2 数据治理: 数据资产目录（敏感四档）+ 质量规则（手动登记）
REM   ✅ F3 试用到期拦截（商用硬伤修复）: 登录拦截 40003 + 临期 7/3/1 提示
REM      + 派生/编排/retry 三入口防绕过 + platform_admin 恢复路径
REM   ✅ V6 迁移: 4 新表 + 12 权限原子（1059~1070）+ 36 授权行（2134~2169）
REM      纯 IF NOT EXISTS + INSERT IGNORE，零 ALTER（任意库重放安全）
REM   ✅ 安全修复: XSS sanitize 公共 JS / D-9 占位瘦身 / U2 两层防提权
REM
REM 【MySQL 自适应】（Docker Desktop 不可用的新环境）
REM   自动探测：docker 容器 eaiselp-mysql 可用 → 走 docker
REM             不可用 → 走原生 MySQL（D:\eaiselp\mysql-8.0，需先跑
REM             deploy\setup_mysql_native.bat 一次性搭建）
REM
REM 环境变量（全部可选）：
...
echo ═══════════════════════════════════════════════════════
echo   eAISEDP 部署 #22 L2治理收口（标准库/数据治理/到期拦截）
echo   %date% %time%
echo ═══════════════════════════════════════════════════════
echo.
